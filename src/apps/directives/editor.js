...
md.directive('editor', ["$rootScope", "SweetAlert", "$state","$parse",
    function ($rootScope, SweetAlert, $state, $parse) {
        return {
            restrict: 'A',
            scope: {
                editor: "=",
            ...
            replace: true,
            template: '<div class="editorBox"></div>',
            link: function (scope, el, attrs) {
                scope.editorObj = new wangEditor(el);
                scope.editorObj.config.menus = ["source", "|", "bold", "underline", "italic", "strikethrough", "eraser", "forecolor", "bgcolor", "|", "quote", "fontfamily", "fontsize", "head2", "unorderlist", "orderlist", "alignleft", "aligncenter", "alignright", "lineheight", "indent", "|", "link", "unlink", "table", "emotion", "|", "img", "video", "location", "insertcode", "|", "undo", "redo"];
                scope.editorObj.config.emotions={
                    'default':{
                        title: '默认',
                        data:[{
                ...
                scope.editorObj.config.familys=[
                    '宋体', '黑体', '楷体', '微软雅黑','仿宋','仿宋_GB2312','方正小标宋简体',
                    'Arial', 'Verdana', 'Georgia',
                    'Times New Roman', 'Microsoft JhengHei',
                    'Trebuchet MS', 'Courier New', 'Impact', 'Comic Sans MS', 'Consolas'
                ];
                scope.editorObj.config.fontsizes={
                    // 格式：'value': 'title'
                    1:"六号",
                    2:"五号",
                    3:"四号",
                ...
                scope.editorObj.create();
                var num = 0, time = 0, key = 0, changeKey = false;
                scope.$watch("changerKey", function () {
                    scope.editorObj.$txt.html(scope.editor);
                    clear();
                    key = 0;
                });
                ...
                $rootScope.$on("overEditor", function () {
                    changeKey = false;
                });
                $rootScope.$on('$stateChangeStart', function (event, toState, url) {
                    if (changeKey) {
                        clear();
                        SweetAlert.swal({
                                title: "是否保存?",
                ...
                scope.editorObj.onchange = function () {

                    if (!time && key) {
                        setTime();
                    }
                    if (key) changeKey = true;
                    key++;
                    // 编辑区域内容变化时，实时打印出当前内容
                    num++;
                    if (num == 10) {
                ...
                obser.observe($(el).parents('.container')[0], {attributes: true});//开始监听
                $(el).css({'padding-top': $(el).prev().height() + 10});
